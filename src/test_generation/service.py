...
def _iter_json_candidates(text: str) -> List[str]:
    ...
    for index, char in enumerate(text):
        ...
        if char == "{":
            ...
            if stack_depth > 0:
                stack_depth -= 1
                if stack_depth == 0 and start_index >= 0:
    ...
    for candidate in candidates:
        try:
            ...
            if isinstance(parsed, dict):
                return parsed
            parse_errors.append("parsed JSON candidate is not an object")
        except json.JSONDecodeError as error:
            parse_errors.append(f"{error}. Hint: use strict JSON literals only, e.g. null/true/false, not None/True/False")
            continue
    raise ValueError("Failed to parse model JSON output: " + " | ".join(parse_errors[:3]))
...
def _validate_function_call_schema(parsed: Dict[str, Any], point: Dict[str, Any]) -> None:
    target_signature = _require_dict(parsed.get("target_signature"), "target_signature")
    function_contract = _require_dict(parsed.get("function_contract"), "function_contract")
    assertions = _require_list(parsed.get("assertions"), "assertions")
    if not str(target_signature.get("entry_name", "")).strip():
        raise ValueError("target_signature.entry_name must be non-empty for function_call")
    parameters = _require_list(target_signature.get("parameters"), "target_signature.parameters")
    for index, parameter in enumerate(parameters):
        if not isinstance(parameter, dict):
            raise ValueError(f"target_signature.parameters[{index}] must be object")
    _require_list(function_contract.get("parameter_order", []), "function_contract.parameter_order")
    _require_list(function_contract.get("notes", []), "function_contract.notes")
    point_text = str(point.get("point_text", ""))
    for index, assertion in enumerate(assertions):
        if not isinstance(assertion, dict):
            raise ValueError(f"assertions[{index}] must be object")
        call = _require_dict(assertion.get("call"), f"assertions[{index}].call")
        expectation = _require_dict(assertion.get("expectation"), f"assertions[{index}].expectation")
        _require_list(call.get("args", []), f"assertions[{index}].call.args")
...
def _validate_test_spec(parsed: Dict[str, Any], point: Dict[str, Any]) -> None:
    expected_point_id = str(point.get("point_id", ""))
    actual_point_id = str(parsed.get("point_id", ""))
...
def _build_point_spec(parsed: Dict[str, Any], point: Dict[str, Any], language: str, raw_output: str) -> RequirementPointTestSpec:
    _validate_test_spec(parsed, point)
    return RequirementPointTestSpec(
        point_id=str(parsed.get("point_id") or point.get("point_id", "")),
        point_text=str(point.get("point_text", "")),
        category=str(point.get("category", "")),
        test_kind=str(parsed.get("test_kind", "functional")),
        execution_mode=str(parsed.get("execution_mode", "program_io")),
...
def generate_tests_for_case(
    ...
    for point_index, point in enumerate(points, start=1):
        ...
        point_spec = _build_point_spec(parsed, point, language, raw_output)
